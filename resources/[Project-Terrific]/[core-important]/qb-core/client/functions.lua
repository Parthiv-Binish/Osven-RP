...
function QBCore.Functions.Notify(text, texttype, length)
    ...
    local typeMap = { primary = 'info', success = 'success', error = 'error', warning = 'warning' }
    if type(text) == 'table' then
        exports.ox_lib:notify({
            title = text.caption or text.text,
            description = type(text) == 'table' and text.text or nil,
            type = typeMap[texttype] or texttype or 'info',
        })
    else
        exports.ox_lib:notify({
            description = text,
